  ...
  if (expandedAliasesCount) {
    ...
    _.each(results, function eachExpandedAliasGroup(result){
      _(result.aliases).values().uniq().each(function applyAlasifyForEachExpandedAlias(file){
        this.pipeline.get('deps').unshift(aliasify.configure({
          aliases: result
          , configDir: result.pattern.cwd
          , verbose: true
        })(file))
        // var stream = through.obj(function streamJson(row, enc, next){
        //   console.log('hi', row, file)
        //   this.push(row)
        //   next()
        // })
        // this.pipeline.get('deps').unshift(stream)
      }, this)
    }, this)
  }
  else {
